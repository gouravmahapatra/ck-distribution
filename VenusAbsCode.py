...
'''
                     Main and testing functions below
-------------------------------------------------------------------------------
'''



#spectra = np.load('/Users/gouravmahapatr/Dropbox/PhD/Codes/VenusAbsCode/LineByLine/spectra.npy')
#wav = np.load('/Users/gouravmahapatr/Dropbox/PhD/Codes/VenusAbsCode/LineByLine/wav_lbl.npy')

#spec,abs_spec = kd_spectrum()
#plt.semilogy(spec[0,:,1],abs_spec[0,:,35],ls='dashed')

# convolve the spectra with the instrument response. 
#I_spic1,Q_spic1 = convolve_spectra(spectra,wav,20.0)
#I_spic2,Q_spic2 = convolve_spectra(spectra,wav,-20.0) 
#I_spic3,Q_spic3 = convolve_spectra(spectra,wav,40.0) 
#I_spic4,Q_spic4 = convolve_spectra(spectra,wav,-40.0) 
#I_spic5,Q_spic5 = convolve_spectra(spectra,wav,0.0)  
#
#pol1 = -Q_spic1/I_spic1
#pol2 = -Q_spic2/I_spic2
#pol3 = -Q_spic3/I_spic3
#pol4 = -Q_spic4/I_spic4
#pol5 = -Q_spic5/I_spic5

# call the Gaussian IRF
#var = 0.0005# in microns
#wav = np.load('/Users/gouravmahapatr/Dropbox/PhD/Codes/VenusAbsCode/LineByLine/wav_lbl.npy')
#wav_conv = np.linspace(wav[0],wav[-1],200)
##wav_conv0 = np.array([wav_conv[1]])
#wav_conv0 = wav_conv[0:-1]
#
## load the geometries file
#geos = np.loadtxt('/Users/gouravmahapatr/Dropbox/PhD/Codes/VenusAbsCode/LineByLine/geos.in',skiprows=4)
#spectra = np.load('/Users/gouravmahapatr/Dropbox/PhD/Codes/VenusAbsCode/LineByLine/spectra.npy')
#
##spectra_conv = convolve_spectra_gaussian(spectra,wav,var)
#spectra_conv = convolve_spectra_gaussian(spectra,wav,wav_conv0,var)
##plt.plot(wav,spectra[:,0,1]/max(spectra[:,0,1]))
##fig,ax = plt.subplots(1,1)
#plt.plot(wav_conv0,spectra_conv[:,0,1],label=str(var*1e3)+' nm')
...
def plotter(perch):
    ...
    if perch == 20.0:
        ...
        plt.subplot(121)
        plt.plot(dnu,ch0_3,c=clr,label=label,ls=ls)
        #plt.plot(wav,I_spic5,c=clr,ls=ls,label=label)
        plt.xlabel('d(nu) (cm$^{-1}$)',fontsize='large')
        plt.ylabel('IRF',fontsize='large')
        
        plt.subplot(122)
        plt.plot(wav,pol1,c=clr,ls=ls)
